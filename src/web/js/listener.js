	...
	static updateEvents() {
		api.event.getList(events => {
			...
			if (events.length) {
				...
				document.querySelector('div.title count').innerText = (pastEvents ? pastEvents + '<arrow>↓</arrow>' : '') + (events.length - pastEvents ? (pastEvents ? ' · ' : '') + (events.length - pastEvents) + '<arrow>↑</arrow>' : '');
			} else
